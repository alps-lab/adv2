...
def test_gs(data_arx, cuda):
    ...
    robj = dict(cam_trans_gs=generate_gs((model, imagenet_normalize),
                                         acid_cam_adv_x, yt,
                                         imagenet_resize_postfn,
                                         device=device, batch_size=36),
                img_x=data_arx['img_x'],
                acid_adv_cam=data_arx['acid_cam_cam'],
                acid_adv_x=acid_cam_adv_x)
    return robj
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', choices=['cuda', 'gpu'], dest='device')
