...
from rev2.isic.isic_model_srelu import get_isic_model_on_resnet50_srelu
...
def attack_batch(config, model_tup, ref_model_tup, bx, by, bm):
    ...
    for c, num_step in zip(config.cs, config.steps):
        for i in range(num_step):
            bx_p = pre_fn(bx)
            logit = model(bx_p)
            by_one = torch.full((n, 7), 0.05 / 6, device='cuda')
            by_one = by_one.scatter(1, by.view(n, -1), 0.95)
            adv_loss = (-by_one * F.log_softmax(logit)).sum() #F.nll_loss(F.log_softmax(logit), by, reduction='sum')
            ...
            if i % 10 == 0:
                ...
                mask = np.logical_and(prob > 0.875, now_dist < best_dist)
...
def main(config):
    ...
    ref_model = get_isic_model_on_resnet50_srelu(False, True, ckpt_path=ISIC_RESNET50_CKPT_PATH)
    ref_model.to('cuda')
    ref_model_tup = (ref_model, identity)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--steps', dest='steps', type=int, nargs='+', default=[401, 201, 201, 101])
