...
def do_int_attack(cam_model, x, cx, target_label, mask_init,
    ...
    for step in range(steps):
        ...
        logits, mask = cam_model(imagenet_normalize(cx), y)
...
def attack_one(cam_model, path, true_label, target_label=None, eps=0.02, debug=False,
    ...
    res["cam_x"] = cam_x
    if debug:
        # vis.image(np.uint8(mask_x[None] * 255.), win="mask_x_%s" % name,
        #           opts=dict(title="mask_x_%s" % name))
    ...
    if debug:
        # vis.image(np.uint8(mask_adv_x[None] * 255.), win="mask_adv_x_%s" % name,
        #           opts=dict(title="mask_adv_x_%s" % name))
        vis.image(np.uint8(255. * cam_adv_x), win="cam_adv_x_%s" % name,
                  opts=dict(title="cam_adv_x_%s" % name))

    best_cx, best_prod, best_int_loss = do_int_attack(cam_model, x, x_adv, target_label,
    ...
    if debug:
        ...
        vis.text("best_int_loss: %.6f" % best_int_loss, win="status_%s" % name,
                 append=True)
    mask_intadv_x = to_mask(best_prod.detach().cpu().numpy()[0])
    cam_intadv_x = make_cam(best_cx_np, mask_intadv_x)
    res["best_intadv_mask"] = mask_intadv_x[None]
    res["best_intadv_cam"] = cam_intadv_x
    if debug:
        # vis.image(np.uint8(mask_intadv_x[None] * 255.), win="mask_intadv_x_%s" % name,
        #           opts=dict(title="mask_intadv_x_%s" % name))
