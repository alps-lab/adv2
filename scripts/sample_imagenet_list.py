...
def main(flags):
    ...
    src_paths = sorted(Path(flags.src_dir).rglob("n*.JPEG"))
    src_paths_2 = sorted(Path(flags.src_dir).rglob("*.JPEG"))
    print(len(src_paths), len(src_paths_2))
    counter = Counter()
    for src_path in src_paths:
        cid = src_path.name.split("_")[0]
        counter[cid] += 1
    ...
    with csv_path.open("w") as csv_file:
        ...
        for picked_path in progressbar(picked_paths):
            cid = picked_path.name.split("_")[0]
            label = str(imagenet_labels[cid])
            writer.writerow([picked_path.name, label])
            copyfile(str(picked_path), str(dest_dir_path.joinpath(picked_path.name)))
