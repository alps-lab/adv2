...
# CUDA_VISIBLE_DEVICES=0 python expr_vis/vis_isic_cam.py /home/xinyang/Data/intattack/rev1/benign_maps/isic_cam_benign/fold_1.npz /home/xinyang/Data/intattack/rev1/benign_maps/isic_cam_acid/fold_1.npz
...
vis = visdom.Visdom(env='isic_map_cam', port=7777)
...
def main(config):
    ...
    img_x, img_y, img_yt = dobj_cam_benign['img_x'].copy(), dobj_cam_benign['img_y'].copy(), dobj_cam_benign['img_yt'].copy()
    ...
    imagenet_labels = load_imagenet_labels()
    tar_img = dobj_cam_benign['img_x'].copy()

    vis_steps = [1200]
    arrs_imgs = [dobj_cam_adv['pgd_s2_step_%d_adv_x' % vis_step].copy() for vis_step in vis_steps]
    arrs_flags = [dobj_cam_adv['pgd_s2_step_%d_adv_succeed' % vis_step].copy() for vis_step in vis_steps]
    arrs_cams = [dobj_cam_adv['pgd_s2_step_%d_adv_cam_n' % vis_step].copy() for vis_step in vis_steps]
    ...
    for i in range(100):
        ...
        vis.images(tar_img[index],
                   win='acid_cam_benign_%d' % index,
                   opts=dict(title='acid_benign_%d, true label: %s, target label: %s' % (index, imagenet_labels[y][1], imagenet_labels[yt][1])))
        vis.images(m3,
                   win='acid_cam_benign_map_%d' % index,
                   opts=dict(title='acid_cam_benign_map_%d,  true label: %s, target label: %s' % (index, imagenet_labels[y][1], imagenet_labels[yt][1])))

        vis.images(imgs,
                   win='img_adv_%d' % index,
                   opts=dict(title='acid_cam_adv_img_%d_resnet, true label: %s' % (index,  imagenet_labels[y][1])))
        vis.images(m2,
                   win='img_%d' % index,
                   opts=dict(title='acid_cam_map_img_%d_resnet, true label: %s' % (index,  imagenet_labels[y][1])))
