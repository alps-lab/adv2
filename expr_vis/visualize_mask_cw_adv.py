...
vis = visdom.Visdom(env='exprmaskadvviscw')
...
def main(config):
    ...
    arrs_masks = [dobj_mask_adv['cw_conf_%d_adv_mask_yt' % vis_conf].copy() for vis_conf in vis_confs]
    ...
    for i in range(60):
        ...
        texts.append('benign')
        cams.append(resize(transform_cam(img_x[index], 1 - mask_benign_y[index])))
        ...
        for j in range(len(arrs_imgs)):
            ...
            if flag == 0:
                ...
                cams.append(resize(transform_cam(arrs_imgs[j][index], 1 - arrs_masks[j][index])))
