...
vis = visdom.Visdom(env='den_cam_target', port=7777)
...
def main(config):
    ...
    cam_benign_y = dobj_cam_benign['target_cam_n'].copy()
    indices = np.random.RandomState(100).choice(len(img_x), size=100, replace=False)
    imagenet_labels = load_imagenet_labels()
    tar_img = dobj_cam_benign['target_images'].copy()
    ...
    for i in range(100):
        ...
        vis.images(tar_img[index],
                   ...
                   opts=dict(title='acid_cam_tar_%d, true label: %s, target label: %s' % (index, imagenet_labels[y][1], imagenet_labels[yt][1])))
        vis.images(m3,
                   win='acid_cam_adv_tar_map_%d' % index,
                   opts=dict(title='acid_cam_adv_tar_map_%d,  true label: %s, target label: %s' % (index, imagenet_labels[y][1], imagenet_labels[yt][1])))
