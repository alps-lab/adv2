...
# CUDA_VISIBLE_DEVICES=0 python expr_vis/new_mask/mask_resnet_reg_pgd.py /home/xinyang/Data/intattack/fixup1/resnet50_mask/fold_1.npz /home/ningfei/xinyang/data_mask/reg_pgd_resnet/fold_1.npz

# CUDA_VISIBLE_DEVICES=0 python expr_vis/new_mask/mask_resnet_reg_pgd.py /home/xinyang/Data/intattack/fixup1/densenet_data/fold_1.npz /home/xinyang/Data/intattack/fixup1/densenet169_regular_pgd/fold_1.npz /home/xinyang/Data/intattack/fixup1/densenet169_mask/fold_1.npz /home/ningfei/xinyang/data_mask/reg_pgd_densenet/fold_1.npz
import argparse
import os
import numpy as np
import cv2
...
vis = visdom.Visdom(env='mask_densenet_reg', port=7777)
...
def main(config):
    ...
    tar_img = dobj_mask_benign['mask_benign_y'].copy()
    ...
    for i in range(100):
        ...
        #                    opts=dict(title='acid_mask_adv_tar_map_%d,  true label: %s, target label: %s' % (i, imagenet_labels[y][1], imagenet_labels[yt][1])))
                vis.images(img,
                        win='reg_pgd_adv_%d' % i,
                        opts=dict(title='reg_pgd_adv_%d,  true label:, target label:' % (i)))
                vis.images(m1,
                        win='reg_map_pgd_adv_%d' % i,
                        opts=dict(title='reg_map_pgd_adv_%d,  true label:, target label:' % (i)))
