...
def main(config):
    split_arx = np.load(config.split_path)
    x, y = split_arx['val_benign_x'], split_arx['val_benign_y']

    if config.model == 'resnet50':
        trained_model = pickle.load(open(RESNET50_MODEL_V2_PATH, 'rb'))
    if config.model == 'densenet169':
    ...
    final_results = {}
    n = len(x)
    wrapper = Wrapper(model, pre_fn, x, None, config.batch_size, config.device == 'gpu')
    for c1, c2, c3 in product(BIT_DEPTHS, MEDIAN_SMOOTHING, NONLOCAL_MEAN):
        ...
        for i in range(n):
            recorder.append(RECORDER_BENIGN, flag[i], '')
        final_results['benign_val', key] = recorder.produce_stats()
        del recorder
