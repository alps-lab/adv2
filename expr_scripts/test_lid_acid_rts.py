...
def main(config):
    adv_x_template = 'pgd_s2_step_%d_adv_x'
    adv_succeed_template = 'pgd_s2_step_%d_adv_succeed'
    ...
    for fold_num in config.folds:
        ...
        # adapt here
        adv_arx = np.load(os.path.join(config.adv_dir, 'resnet_rts_acid_pgd_fold_resnet_%d.npz' % fold_num))
