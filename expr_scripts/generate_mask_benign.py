...
def main(config):
    ...
    mask_benign_y = generate_masks(mask_config, model_tup, (img_x, img_y), config.device == 'gpu')
    mask_benign_yt = generate_masks(mask_config, model_tup, (img_x, img_yt), config.device == 'gpu')
    np.savez(config.save_path, mask_benign_y=mask_benign_y, mask_benign_yt=mask_benign_yt)
