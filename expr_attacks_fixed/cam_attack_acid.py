...
# CUDA_VISIBLE_DEVICES=0 python expr_attacks_fixed/cam_attack_acid.py /home/xinyang/Data/intattack/rev1/target_maps/cam/fold_1.npz /home/ningfei/xinyang/data_fix1/target/cam/fold_1.npz -c 10.0 -b 30 --s2-iters 1400
...
def attack_batch(config, model_tup, forward_tup, batch_tup, cam_benign):
    ...
    for i in range(config.s1_iters):
        ...
        loss = F.nll_loss(F.log_softmax(logits, -1), by, reduction='sum')
...
def attack(config):
    ...
    img_x, img_yt = cam_benign_arx['img_x'].copy(), cam_benign_arx['img_yt'].copy()
    cam_benign_y = cam_benign_arx['target_cam_n'].copy()
