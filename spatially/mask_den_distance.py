...
#
# path_tar = ['/home/xinyang/Data/intattack/rev1/target_maps/mask_resnet50/fold_1.npz','/home/xinyang/Data/intattack/rev1/target_maps/mask_resnet50/fold_2.npz']
# #
...
reg = []
d = []
for i in range(len(path_acid)):
    acid.append(np.load(path_acid[i]))
    val.append(np.load(path_acid[i]))
...
for i in range(len(path_acid)):
    ...
    for j in range(len(acid[i]['pgd_s2_step_1000_adv_mask_m'])):
        distance = 99999999999.0
        best = 0
        for step in range(50,1050,50):
            if acid[i]['pgd_s2_step_%d_succeed_disc' % step][j] == 1:
                tmp_mask_tar = acid[i]['mask_benign_y'][j].copy()
                tmp_mask_acid = acid[i]['pgd_s2_step_%d_adv_mask_m' % step][j].copy()
    ...
    for j in range(len(acid[i]['pgd_s2_step_1000_adv_mask_mnow_disc'])):
        ...
        if best_step[j] != 0:
            ...
            if acid[i]['pgd_s2_step_%d_succeed_disc' % best_step[j]][j] == 1:
                ...
                con_acid.append(pro(acid[i]['pgd_s2_step_%d_adv_logits_disc' % best_step[j]][j].copy()))
                b_a.append(acid[i]['mask_benign_y'][j].copy())
                a.append(acid[i]['pgd_s2_step_%d_adv_mask_m' % best_step[j]][j].copy())
