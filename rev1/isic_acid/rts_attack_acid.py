...
def attack_batch(config, rts_model, batch_tup, rts_benign):
    ...
    bx, by, m0 = (torch.tensor(bx_np, device=device),
                  torch.tensor(by_np, device=device), torch.tensor(rts_benign, device=device))
    ...
    for i in range(config.s2_iters):
        c_now += c_inc
        rts, rts_logits = rts_model.saliency_fn(bx_adv, by, model_confidence=0., return_classification_logits=True)
        diff = rts - m0
...
def attack(config):
    ...
    save_dobj['saliency_benign_y'] = rts_benign
