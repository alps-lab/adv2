...
def attack(config):
    model_tup, forward_tup = cam_resnet50()
    cam_target_arx = np.load(config.cam_target_path)
    img_x, img_yt = cam_target_arx['img_x'].copy(), cam_target_arx['img_yt'].copy()
    cam_benign_y = cam_target_arx['target_cam_n'].copy()
    ...
    save_dobj['target_images'] = cam_target_arx['target_images']
    save_dobj['img_y'] = cam_target_arx['img_y']
    np.savez(config.save_path, **save_dobj)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('cam_target_path')
