...
def attack_batch(config, rts_model, batch_tup, rts_benign):
    ...
    for i in range(config.s2_iters):
        ...
        total_loss = 2.0 * adv_loss + 0.15 * rts_adv_loss + tau * flow_loss + c_now * loss_rts
...
def attack(config):
    ...
    rts_target_arx = np.load(config.rts_target_path)
    img_x, img_y, img_yt = (rts_target_arx['img_x'].copy(), rts_target_arx['img_y'].copy(),
                            rts_target_arx['img_yt'].copy())
    rts_benign = rts_target_arx['target_rts'].copy()
    ...
    save_dobj['target_images'] = rts_target_arx['target_images']
    save_dobj['img_y'] = rts_target_arx['img_y']
    np.savez(config.save_path, **save_dobj)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('rts_target_path')
    ...
    parser.add_argument('--tau', dest='tau', type=float, default=0.002)
